fn main() {
    ...
    // Rust puts a lot of emphasis in immutability. Immutability while keeping Concurrency in mind.

    // We use the 'let' Key-word to declare variables in Rust which is very similar to what Javascript does now, making things pretty easy
    ...
    // It is recommended or wished that you use 'Snake Case' in Rust and you will be prompted every now and then to do so too

    // Rust will also complains if you try to run or compile a program with un-used variables
    ...
    // These are the basic Types. There are more, its suggested to go through them once but the ones mentioned here were the simplest ones
